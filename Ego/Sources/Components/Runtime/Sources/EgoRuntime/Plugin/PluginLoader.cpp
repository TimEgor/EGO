...
namespace
{
    ego::FileName SelectPluginModuleFromCatalog(const char* _typeName)
    {
        ...
        const ego::PluginCatalogPointer pluginCatalog = runtimeContext->getPluginCatalogPointer();
        if (!pluginCatalog)
        {
            return ego::FileName();
        }
...
void ego::PluginLoader::initModule(void* _moduleHandle, const PluginModuleInfo& _moduleInfo)
{
    ...
    const context::ContextStackInterfacePointer contextStack = context::ContextStackCore::GetInstance().getStackPointer();
    EGO_CHECK_RETURN(contextStack);
    ...
    return initFunction(_moduleInfo, contextStack);
}
