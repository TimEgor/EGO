...
{
    context::ContextScopePointer contextScope = nullptr;
    {
        const JobControllerPointer jobController = _jobController.lock();
        if (!jobController)
        {
            return;
        }

        contextScope = jobController->m_contextScope;
    }

    context::ScopedContextScope scopedContextScope(contextScope);
...
bool ego::JobController::init(uint32_t _threadCount, const context::ContextScopePointer& _contextScope, const char* _name)
{
    (void)_name;

    EGO_CHECK_RETURN_FALSE(_contextScope);
    ...
    {
        std::thread thread(
            [jobController = weakFromThis()]()
            {
                ThreadFunction(jobController);
            });

        if (_name)
        {
            const std::string threadName = StringFormat("{}_{}", _name, threadIndex);
            threadUtils::SetThreadName(thread.native_handle(), threadName.c_str());
        }
