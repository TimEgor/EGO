...
namespace ego
{
    ...
    namespace literals
    {
        constexpr ComputeValueType operator ""_ecv(long double _value) { return static_cast<ComputeValueType>(_value); }
    }

    namespace math
    {
        template <typename T>
        constexpr T TypedPi() { return std::numbers::pi_v<T>; }
        ...
        template <typename T>
        T ConvertDegToRad(T _angle) { return _angle * TypedDegToRad<T>(); }

        template <typename T>
        T ConvertRadToDeg(T _angle) { return _angle * TypedRadToDeg<T>(); }
        ...
        template <typename T>
        bool IsApproxLessOrEqual(T _value1, T _value2, T _epsilon = TypedEpsilon<T>()) // is value1 less or equal than value2
        {
            EGO_ASSERT(_epsilon > T(0.0));
            return (_value2 - _value1) >= _epsilon;
        }
        ...
        template <typename T>
        bool IsApproxGreaterOrEqual(T _value1, T _value2, T _epsilon = TypedEpsilon<T>()) // is value1 greater or equal than value2
        {
            EGO_ASSERT(_epsilon > T(0.0));
            return (_value1 - _value2) >= _epsilon;
        }
