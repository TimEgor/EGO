...
{
	return m_minPosition.isLessOrEqual(_volume.m_minPosition) &&
		m_maxPosition.isGreaterOrEqual(_volume.m_maxPosition);
}

bool ego::AABB3::isOverlapped(const AABB3& _volume) const
{
	return !(m_maxPosition.isAnyLess(_volume.m_minPosition)) ||
		m_minPosition.isAnyGreater(_volume.m_maxPosition);
}
