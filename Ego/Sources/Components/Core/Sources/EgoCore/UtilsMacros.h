...
#define EGO_CHECK_CALL(_CHECK_VAL, _FUNCTION) if (!(_CHECK_VAL)) { _FUNCTION; }
#define EGO_CHECK_INITIALIZATION(_CHECK_VAL) if (!(_CHECK_VAL)) { release(); return false; }
#define EGO_CHECK_RETURN_VALUE(_CHECK_VAL, _VALUE) if (!(_CHECK_VAL)) { return _VALUE; }
#define EGO_CHECK_RETURN(_CHECK_VAL) if (!(_CHECK_VAL)) { return; }
