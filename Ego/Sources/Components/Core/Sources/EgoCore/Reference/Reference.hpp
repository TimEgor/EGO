...
{
    static_assert(std::is_base_of_v<MTCountable, T>, "ego::Reference<T> requires T to derive from ego::MTCountable");
}
...
{
    return static_cast<const void*>(_reference1.getObject()) == static_cast<const void*>(_reference2.getObject());
}
...
bool ego::operator<(const Reference<T1>& _reference1, const Reference<T2>& _reference2)
{
    return std::less<const void*>()(static_cast<const void*>(_reference1.getObject()), static_cast<const void*>(_reference2.getObject()));
}
