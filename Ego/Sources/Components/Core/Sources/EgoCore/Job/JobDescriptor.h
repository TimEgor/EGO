...
namespace ego
{
    ...
    std::enable_if_t<std::is_invocable_r_v<JobReference, TFactory>, JobDescriptor>
        CreateJobDescriptor(TFactory _factory)
    {
        return CreateJobDescriptor(JobDescriptor::JobFactory(std::move(_factory)));
    }

    template <typename TJob, typename... TArgs>
    std::enable_if_t<std::is_base_of_v<Job, TJob>, JobDescriptor>
        CreateJobDescriptor(TArgs&&... _args)
    {
        static_assert(!std::is_abstract_v<TJob>, "Job type must not be abstract.");
        static_assert(
            std::is_constructible_v<TJob, std::decay_t<TArgs>&...>,
            "Job type isn't constructible from stored arguments."
        );
        static_assert(
            (std::is_copy_constructible_v<std::decay_t<TArgs>> && ...),
            "Job descriptor arguments must be copy constructible."
        );
