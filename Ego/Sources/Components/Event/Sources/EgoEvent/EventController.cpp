...
ego::EventCallbackID ego::EventController::addEventCallback(EventType _type, const EventCallback& _callback)
{
    ...
    dispatcherInfo.m_elementPtr->m_orderIndex = static_cast<uint32_t>(findEventIter->second.size() - 1);
...
bool ego::EventController::unregisterInstancedEvent(InstancedEventID _eventID)
{
    ...
    {
        const InstancedEventCallbackID prevDispatcherID = m_instancedCallbacks.getElement(dispatcherID)->m_prevSiblingID;
        m_instancedCallbacks.removeElement(dispatcherID);

        dispatcherID = prevDispatcherID;
    }

    m_instancedCallbackOrders.erase(findEventIter);
...
bool ego::EventController::emitEvent(InstancedEventID _eventID, const Event& _event) const
{
    ...
    while (dispatcherID != InvalidInstancedEventCallbackID)
    {
        ...
        dispatcherID = dispatcherData->m_prevSiblingID;
    }
