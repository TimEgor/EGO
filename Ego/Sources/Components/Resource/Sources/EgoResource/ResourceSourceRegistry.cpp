...
bool ego::ResourceSourceRegistry::addResourceProvider(const FileName& _extension, const ResourceProviderPointer& _provider)
{
    ...
    const auto [providerIt, added] = m_resourceProviders.emplace(extension, _provider);
    return added;
}

bool ego::ResourceSourceRegistry::removeResourceProvider(const FileName& _extension, const ResourceProviderPointer& _provider)
{
    const std::string extension = NormalizeProviderExtension(_extension);
    if (extension.empty() || !_provider)
    {
    ...
    if (providerIt == m_resourceProviders.end() || providerIt->second.get() != _provider.get())
    {
        return false;
    }

    m_resourceProviders.erase(providerIt);
    return true;
}
