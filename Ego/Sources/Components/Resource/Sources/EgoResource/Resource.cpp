...
ego::ResourceDeleter::ResourceDeleter(const WeakPointer<ResourceController>& _controller)
...
void ego::ResourceDeleter::operator()(Resource* _resource) const
{
    ...
    const SharedPointer<ResourceController> controller = m_controller.lock();
    if (controller)
    {
        ResourceController::ResourceControllerAccessor::RemoveResource(*controller, _resource);
    }
