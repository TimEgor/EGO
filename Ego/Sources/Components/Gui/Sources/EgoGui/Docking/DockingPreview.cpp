...
namespace
{
    constexpr size_t SpaceTargetCount = 5;
    constexpr size_t RootTargetCount = 4;
    constexpr size_t RootTargetOffset = SpaceTargetCount;
} // namespace
    ...
    const DockingStyle& _style)
{
    ...
    if (_showRootTargets)
    {
        ...
        preview.m_targets[RootTargetOffset].m_bounds = Rect(_rootBounds.getLeft() + targetInset, center.m_y - targetSize * 0.5f, targetSize, targetSize);
        preview.m_targets[RootTargetOffset + 1].m_bounds =
            Rect(_rootBounds.getRight() - targetInset - targetSize, center.m_y - targetSize * 0.5f, targetSize, targetSize);
        preview.m_targets[RootTargetOffset + 2].m_bounds = Rect(center.m_x - targetSize * 0.5f, _rootBounds.getTop() + targetInset, targetSize, targetSize);
        preview.m_targets[RootTargetOffset + 3].m_bounds =
            Rect(center.m_x - targetSize * 0.5f, _rootBounds.getBottom() - targetInset - targetSize, targetSize, targetSize);
    }

    const float targetHitPadding = (std::max)(2.0f, _style.m_targetSpacing * 0.5f);
    for (size_t targetIndex = 0; targetIndex < SpaceTargetCount; ++targetIndex)
    {
        DockingTarget& target = preview.m_targets[targetIndex];
        target.m_level = DockingTargetLevel::Space;
        target.m_placement = GetSpacePlacement(targetIndex);
        target.m_isVisible = showSpaceTargets;
        ...
        const bool isHorizontal = target.m_placement == DockingPlacement::Left || target.m_placement == DockingPlacement::Right;
        const bool isVertical = target.m_placement == DockingPlacement::Top || target.m_placement == DockingPlacement::Bottom;
        const bool isAvailableAfterOriginRemoval =
            (isHorizontal && _allowHorizontalSpaceSplitAfterOriginRemoval) || (isVertical && _allowVerticalSpaceSplitAfterOriginRemoval);
        target.m_isAvailable = isAvailableAfterOriginRemoval || IsPlacementAvailable(_targetSpaceBounds, target.m_placement, _style);
    ...
    for (size_t targetIndex = 0; targetIndex < RootTargetCount; ++targetIndex)
    {
        DockingTarget& target = preview.m_targets[RootTargetOffset + targetIndex];
        target.m_level = DockingTargetLevel::Root;
        target.m_placement = GetRootPlacement(targetIndex);
        target.m_isVisible = _showRootTargets;
        ...
        target.m_isAvailable = IsRootPlacementAvailable(_rootBounds, _rootMinimumSize, target.m_placement, _style);
        target.m_hitBounds = Rect(
            target.m_bounds.m_position.m_x - targetHitPadding,
            target.m_bounds.m_position.m_y - targetHitPadding,
            target.m_bounds.m_size.m_x + targetHitPadding * 2.0f,
    ...
    for (size_t targetIndex = preview.m_targets.size(); targetIndex > 0; --targetIndex)
    {
        const DockingTarget& target = preview.m_targets[targetIndex - 1];
        if (!target.m_isVisible || !target.m_isAvailable || !target.m_hitBounds.contains(_pointerPosition))
        {
            continue;
        }
        ...
        preview.m_isDropAllowed = true;
        if (target.m_level == DockingTargetLevel::Root)
        {
...
ego::gui::DockingPlacement ego::gui::DockingPreviewBuilder::GetSpacePlacement(size_t _targetIndex)
{
    static constexpr std::array<DockingPlacement, SpaceTargetCount> Placements = {DockingPlacement::Center,
...
ego::gui::DockingPlacement ego::gui::DockingPreviewBuilder::GetRootPlacement(size_t _targetIndex)
{
    static constexpr std::array<DockingPlacement, RootTargetCount> Placements = {DockingPlacement::Left,
        DockingPlacement::Right,
        DockingPlacement::Top,
        DockingPlacement::Bottom};
    EGO_ASSERT(_targetIndex < Placements.size());
