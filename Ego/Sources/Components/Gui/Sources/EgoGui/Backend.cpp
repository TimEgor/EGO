...
ego::gui::Backend::Backend() = default;

ego::gui::Backend::~Backend()
{
    release();
}

bool ego::gui::Backend::init(const ViewportProviderPointer& _viewportProvider, bool _enableMultiViewport)
{
    ...
    {
        const ContextScope contextScope(*m_context);
...
bool ego::gui::Backend::setFont(const FileContent& _content, float _size)
{
...
bool ego::gui::Backend::setStyle(const GuiStyle& _style)
{
    EGO_CHECK_RETURN_FALSE(m_isInitialized && m_context && !m_isFrameActive);
...
bool ego::gui::Backend::beginFrame(float _deltaTime)
{
    EGO_CHECK_RETURN_FALSE(m_isInitialized && m_context && !m_isFrameActive);

    const ContextScope contextScope(*m_context);
    EGO_CHECK_RETURN_FALSE(m_platformAdapter.beginFrame());
...
bool ego::gui::Backend::endFrame(GuiRenderData& _renderData)
{
    EGO_CHECK_RETURN_FALSE(m_context && m_isFrameActive);
...
void ego::gui::Backend::cancelFrame()
{
    EGO_CHECK_RETURN(m_context && m_isFrameActive);
...
ego::gui::GuiFrameTextureID ego::gui::Backend::bindTexture(const gpu::TextureViewPointer& _textureView, TextureSamplingMode _samplingMode)
{
...
bool ego::gui::Backend::isInitialized() const
{
    return m_isInitialized;
}

ImGuiContext& ego::gui::Backend::getContext() const
{
    EGO_ASSERT(m_context);

    return *m_context;
}

bool ego::gui::Backend::release()
{
...
bool ego::gui::Backend::initializeContext(bool _enableMultiViewport)
{
...
bool ego::gui::Backend::releaseContext()
{
    if (!m_context)
    {
        return true;
...
void ego::gui::Backend::resetState()
{
