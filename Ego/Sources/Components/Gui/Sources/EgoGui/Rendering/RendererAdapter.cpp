...
bool ego::gui::RendererAdapter::init()
{
...
void ego::gui::RendererAdapter::release()
{
...
void ego::gui::RendererAdapter::beginFrame()
{
    EGO_ASSERT(m_isInitialized);
    m_frameTextures.reset();
}

ego::gui::GuiFrameTextureID ego::gui::RendererAdapter::bindTexture(const gpu::TextureViewPointer& _textureView, TextureSamplingMode _samplingMode)
{
...
bool ego::gui::RendererAdapter::buildRenderData(const PlatformAdapter& _platform, GuiRenderData& _renderData)
{
    EGO_CHECK_RETURN_FALSE(m_isInitialized);

    const PlatformRenderTargetCollection renderTargets = _platform.getRenderTargets();
    _renderData.m_viewports.reserve(renderTargets.size());
    for (const PlatformRenderTarget& renderTarget : renderTargets)
    {
...
bool ego::gui::RendererAdapter::bindImGuiTextures(const ImDrawData& _drawData)
{
...
bool ego::gui::RendererAdapter::appendViewport(const ImDrawData& _drawData, const GraphicPresenterPointer& _graphicPresenter, GuiRenderData& _renderData)
{
    EGO_CHECK_RETURN_FALSE(m_isInitialized);
...
ego::gui::Vertex ego::gui::RendererAdapter::ConvertVertex(const ImDrawVert& _vertex, float _displayPositionX, float _displayPositionY)
{
...
ego::gui::TextureSamplingMode ego::gui::RendererAdapter::GetSamplingMode(const ImTextureData& _textureData)
{
    return _textureData.Format == ImTextureFormat_Alpha8 ? TextureSamplingMode::Alpha : TextureSamplingMode::Color;
}

bool ego::gui::RendererAdapter::appendCommand(
