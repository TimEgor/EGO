...
void ego::gui::GuiPaintContext::drawTriangle(
...
{
    appendTriangle(_firstPosition, _secondPosition, _thirdPosition, _color, _textureId);
}
...
void ego::gui::GuiPaintContext::appendCircle(const GuiPosition& _center, float _radius, const GuiColor& _color, GuiTextureID _textureId)
{
    ...
    for (uint32_t segmentIndex = 0; segmentIndex < CircleSegmentCount; ++segmentIndex)
    {
        ...
        const GuiPosition position(_center.m_x + std::cos(angle) * _radius, _center.m_y + std::sin(angle) * _radius);
...
void ego::gui::GuiPaintContext::appendTriangle(
    const GuiPosition& _firstPosition,
    const GuiPosition& _secondPosition,
    const GuiPosition& _thirdPosition,
    const GuiColor& _color,
    GuiTextureID _textureId)
{
    ...
    m_drawData.m_vertices.push_back(GuiVertex{_firstPosition, FloatVector2(0.0f, 0.0f), _color});
    m_drawData.m_vertices.push_back(GuiVertex{_secondPosition, FloatVector2(1.0f, 0.0f), _color});
    m_drawData.m_vertices.push_back(GuiVertex{_thirdPosition, FloatVector2(1.0f, 1.0f), _color});
