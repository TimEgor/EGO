...
ego::gui::GuiEventResult ego::gui::GuiWidget::handleEvent(const GuiInputEvent& _event)
{
    if (_event.m_type != GuiInputEventType::FocusLost && !isVisible())
    {
        return GuiEventResult::Unhandled;
    }

    return onEvent(_event);
}
...
void ego::gui::GuiWidget::setVisibility(GuiVisibility _visibility)
{
    ...
    if (m_visibility == GuiVisibility::Visible && _visibility != GuiVisibility::Visible)
    {
        GuiInputEvent focusLostEvent;
        focusLostEvent.m_type = GuiInputEventType::FocusLost;
        handleEvent(focusLostEvent);
    }
...
ego::gui::GuiEventResult ego::gui::GuiWidget::onEvent(const GuiInputEvent&)
{
    return GuiEventResult::Unhandled;
}
