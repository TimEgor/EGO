...
struct ego::gui::PlatformAdapter::ViewportData final
{
    ViewportID m_viewportID = InvalidViewportID;
};

struct ego::gui::PlatformAdapter::ViewportPollResult final
{
    bool m_isAlive = false;
    bool m_isFocused = false;
};

ego::gui::PlatformAdapter::~PlatformAdapter()
{
    EGO_ASSERT(!m_provider);
}

bool ego::gui::PlatformAdapter::init(const ViewportProviderPointer& _provider)
{
...
void ego::gui::PlatformAdapter::release()
{
    if (!m_provider)
    {
        return;
...
bool ego::gui::PlatformAdapter::beginFrame()
{
...
bool ego::gui::PlatformAdapter::endFrame()
{
    EGO_CHECK_RETURN_FALSE(m_provider && ImGui::GetCurrentContext() == m_context && !m_hasFatalPlatformError);
...
ego::gui::PlatformRenderTargetCollection ego::gui::PlatformAdapter::getRenderTargets() const
{
    PlatformRenderTargetCollection targets;
...
ego::gui::PlatformAdapter::ViewportPollResult ego::gui::PlatformAdapter::pollViewport(ImGuiViewport& _viewport, const ViewportData& _data)
{
...
void ego::gui::PlatformAdapter::destroyViewport(ImGuiViewport& _viewport)
{
...
void ego::gui::PlatformAdapter::updatePrimaryDisplay()
{
...
void ego::gui::PlatformAdapter::markViewportFailure(ImGuiViewport& _viewport)
{
    m_hasFatalPlatformError = true;
    _viewport.PlatformRequestClose = true;
}

ego::gui::ViewportID ego::gui::PlatformAdapter::allocateViewportID()
{
...
ego::gui::PlatformAdapter* ego::gui::PlatformAdapter::GetCurrentAdapter()
{
    ImGuiContext* context = ImGui::GetCurrentContext();
    if (!context)
    {
    ...
    return static_cast<PlatformAdapter*>(ImGui::GetIO().BackendPlatformUserData);
}

ego::gui::PlatformAdapter::ViewportData* ego::gui::PlatformAdapter::GetViewportData(ImGuiViewport* _viewport)
{
...
const ego::gui::PlatformAdapter::ViewportData* ego::gui::PlatformAdapter::GetViewportData(const ImGuiViewport* _viewport)
{
    if (!_viewport || !_viewport->PlatformUserData)
    {
        return nullptr;
...
ego::gui::PlatformAdapter::ViewportDataOwner ego::gui::PlatformAdapter::TakeViewportData(ImGuiViewport& _viewport)
{
...
void ego::gui::PlatformAdapter::PlatformCreateWindow(ImGuiViewport* _viewport)
{
    PlatformAdapter* adapter = GetCurrentAdapter();
...
void ego::gui::PlatformAdapter::PlatformDestroyWindow(ImGuiViewport* _viewport)
{
    if (!_viewport)
    {
        return;
...
void ego::gui::PlatformAdapter::PlatformShowWindow(ImGuiViewport* _viewport)
{
    PlatformAdapter* adapter = GetCurrentAdapter();
...
void ego::gui::PlatformAdapter::PlatformSetWindowPosition(ImGuiViewport* _viewport, ImVec2 _position)
{
    PlatformAdapter* adapter = GetCurrentAdapter();
    ViewportData* data = GetViewportData(_viewport);
    if (!adapter || !data || data->m_viewportID == adapter->m_primaryViewportID)
...
ImVec2 ego::gui::PlatformAdapter::PlatformGetWindowPosition(ImGuiViewport* _viewport)
{
    PlatformAdapter* adapter = GetCurrentAdapter();
...
void ego::gui::PlatformAdapter::PlatformSetWindowSize(ImGuiViewport* _viewport, ImVec2 _size)
{
    PlatformAdapter* adapter = GetCurrentAdapter();
    ViewportData* data = GetViewportData(_viewport);
    if (!adapter || !data || data->m_viewportID == adapter->m_primaryViewportID)
...
ImVec2 ego::gui::PlatformAdapter::PlatformGetWindowSize(ImGuiViewport* _viewport)
{
    PlatformAdapter* adapter = GetCurrentAdapter();
    const ViewportData* data = GetViewportData(_viewport);
    if (!adapter || !data)
...
ImVec2 ego::gui::PlatformAdapter::PlatformGetWindowFramebufferScale(ImGuiViewport*)
{
    return ImVec2(1.0f, 1.0f);
}

void ego::gui::PlatformAdapter::PlatformSetWindowFocus(ImGuiViewport* _viewport)
{
    PlatformAdapter* adapter = GetCurrentAdapter();
    ViewportData* data = GetViewportData(_viewport);
    if (!adapter || !data || data->m_viewportID == adapter->m_primaryViewportID)
...
bool ego::gui::PlatformAdapter::PlatformGetWindowFocus(ImGuiViewport* _viewport)
{
    PlatformAdapter* adapter = GetCurrentAdapter();
    const ViewportData* data = GetViewportData(_viewport);
...
void ego::gui::PlatformAdapter::PlatformSetWindowTitle(ImGuiViewport*, const char*) {}

void ego::gui::PlatformAdapter::PlatformSetWindowAlpha(ImGuiViewport*, float) {}

void ego::gui::PlatformAdapter::PlatformUpdateWindow(ImGuiViewport* _viewport)
{
    PlatformAdapter* adapter = GetCurrentAdapter();
    ViewportData* data = GetViewportData(_viewport);
    if (!adapter || !data || data->m_viewportID == adapter->m_primaryViewportID)
